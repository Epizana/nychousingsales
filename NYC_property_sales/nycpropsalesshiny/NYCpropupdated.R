...
#####CONDOTX: CONDO Apartment Unit ANNUAL RETURN fracture++++++++#########
...
as.data.table(condotx)

#write csv to be read by server.R
write.csv(condotx, 'condotx.csv',row.names = FALSE)

#####BUILDTX: CONDO Apartment Unit ANNUAL RETURN fracture++++++++#########
...
prctblbduil = buildtx
write.csv(prctblbduil, 'prctblbduil.csv', row.names= FALSE)

#continue transforming buildtx
...
write.csv(buildtx, 'buildtx.csv', row.names= FALSE)
